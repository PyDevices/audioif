...
workspace. ``audioinstruments`` is about 236 KB of bytecode at ``opt=3``, which
is worth it on a board actually playing the instruments and pure cost
everywhere else - including on rp2040-class CircuitPython builds, which have
around 1 MB for the whole firmware.

So, unlike its neighbours (``pdwidgets``, ``palettes``), this one is opt-in::

    AUDIOIF_FREEZE_INSTRUMENTS=1 ./build_interpreters.sh --only mp-unix

Everywhere else the library arrives by MIP, from ``<repo>/lib/<package>``, like
the rest of the PyDevices Python tiers. That is also the safer default while
...
def _wanted():
    value = os.environ.get("AUDIOIF_FREEZE_INSTRUMENTS", "").strip().lower()
